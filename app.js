...
// ─── DATA: LOCATIONS ───
const locations = [
  { name: "Clock Tower", cat: "Core Location", lat: 30.3250, lng: 78.0419 },
  { name: "Rajpur Road", cat: "Core Location", lat: 30.3660, lng: 78.0700 },
  { name: "ISBT Dehradun", cat: "Transport Hub", lat: 30.2886, lng: 78.0131 },
  { name: "Railway Station Dehradun", cat: "Transport Hub", lat: 30.3165, lng: 78.0322 },
  { name: "Sahastradhara Road", cat: "Core Location", lat: 30.3873, lng: 78.1310 },
  { name: "Ballupur", cat: "Core Location", lat: 30.3345, lng: 78.0335 },
  { name: "Prem Nagar", cat: "Core Location", lat: 30.3430, lng: 77.9990 },
  { name: "Clement Town", cat: "Core Location", lat: 30.2740, lng: 78.0080 },
  { name: "Dalanwala", cat: "Core Location", lat: 30.3258, lng: 78.0540 },
  { name: "Patel Nagar", cat: "Core Location", lat: 30.3000, lng: 78.0200 },
  { name: "Graphic Era University", cat: "College", lat: 30.2675, lng: 78.0132 },
  { name: "Uttaranchal University", cat: "College", lat: 30.3760, lng: 78.1100 },
  { name: "DIT University", cat: "College", lat: 30.3984, lng: 78.0745 },
  { name: "Max Hospital", cat: "Hospital", lat: 30.3256, lng: 78.0437 },
  { name: "Pacific Mall", cat: "Mall", lat: 30.3665, lng: 78.0708 },
  { name: "Robber's Cave", cat: "Tourist Place", lat: 30.3760, lng: 78.0690 },
  { name: "Mussoorie", cat: "Tourist Place", lat: 30.4598, lng: 78.0644 },
  { name: "Tapkeshwar Temple", cat: "Tourist Place", lat: 30.3420, lng: 78.0110 },
  { name: "Forest Research Institute", cat: "Tourist Place", lat: 30.3433, lng: 77.9990 },
  { name: "Paltan Bazaar", cat: "Market", lat: 30.3253, lng: 78.0417 }
];
...
let trafficLayer;
let feedbackData = [
  { name: "Adarsh", message: "Amazing navigation system! The UI is very premium.", rating: 5, date: new Date().toLocaleString() },
  { name: "Rahul", message: "Helpful for finding shortcuts in Dehradun.", rating: 4, date: new Date().toLocaleString() }
];
...
function sendChat() {
  ...
  setTimeout(() => {
    typing.remove();
    let response = "I'm sorry, I don't recognize that location. Try asking about 'hospitals' or 'colleges'!";
    
    const query = msg.toLowerCase();
    if(query.includes('hospital')) {
      const h = locations.filter(l => l.cat === 'Hospital');
      response = `📍 Found ${h.length} hospitals: ${h.map(x => x.name).join(', ')}.`;
    } else if(query.includes('college') || query.includes('university')) {
      const c = locations.filter(l => l.cat === 'College');
      response = `🎓 Nearby colleges: ${c.map(x => x.name).join(', ')}.`;
    } else if(query.includes('mall')) {
      response = "🛍 Pacific Mall and Crossroad Mall are the best options in Dehradun.";
    } else if(query.includes('hi') || query.includes('hello')) {
      response = "Hello! I am your Dehradun Wayfinder Assistant. How can I help you navigate today?";
    }
